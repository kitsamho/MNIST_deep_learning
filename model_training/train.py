...
def train(model: torch.nn.Sequential, train_loader, cost, optimizer, epoch):
    ...
    for e in range(epoch):
        ...
        print(
            f"Epoch {e}: Loss {running_loss / len(train_loader.dataset)}, Accuracy  {100 * (correct / len(train_loader.dataset))}%")
        running_loss_plot.append(running_loss / len(train_loader.dataset))
        accuracy_plot.append(100 * (correct / len(train_loader.dataset)))
    return running_loss_plot, accuracy_plot